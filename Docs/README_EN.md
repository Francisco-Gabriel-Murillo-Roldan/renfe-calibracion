...
**Important:** Curves generated by the Maxima program (`/Curvas-Traccion/`) are an alternative to manufacturer-documented curves. However, they **must** be validated through the standardised grade test. Experiment #4 demonstrates that the μ₀ used to generate the curve must match the CK parameter A. A mismatch produces unpredictable behaviour in rain and snow.
