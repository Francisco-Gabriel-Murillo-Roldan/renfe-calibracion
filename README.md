# renfe-calibracion

*English version below*

---

## 📌 Resumen

**Español:** Calibración paramétrica de 11 locomotoras diésel RENFE para Open Rails mediante **ethical reverse engineering**, documentación técnica real y pruebas empíricas estandarizadas (Método Teluroncio).

**English:** Parametric calibration of 11 RENFE diesel locomotives for Open Rails through **ethical reverse engineering**, real technical documentation, and standardised empirical testing (Teluroncio Method).

---

**Author:** Francisco Gabriel Murillo Roldán

**Degree:** Mining Engineer

**Methodology:** Teluroncio Method (empirical calibration through standardized testing)

---
## 📁 Estructura del Repositorio | Repository Structure

| Carpeta | Contenido |
|:---|:---|
| 📄 **[README.md](./README.md)** | Resumen ejecutivo bilingüe del proyecto |
| 📄 **[LICENSE](./LICENSE)** | Licencia MIT / MIT License |
| 📁 **[Assets/](./Assets/)** | Logo, paleta de colores corporativos y protección de marca |
| 📁 **[Docs/](./Docs/)** | Documentación completa en español e inglés |
| 📁 **[Fichas-Tecnicas/](./Fichas-Tecnicas/)** | Fichas por locomotora con parámetros finales (11 PDFs) |
| 📁 **[Experimentos/](./Experimentos/)** | Informes detallados de experimentos de calibración |
| 📁 **[Parametros-Clave/](./Parametros-Clave/)** | Tablas resumen comparativas entre series |
| 📁 **[Curvas-Traccion/](./Curvas-Traccion/)** | Programas Maxima/WxMaxima y curvas en CSV y TXT |
| 📁 **[OpenRails-Logs/](./OpenRails-Logs/)** | Logs de pruebas Antes/Después anonimizados |
---

## 📖 Documentación Completa | Full Documentation

- **[README en Español](./docs/README_ES.md)**
- **[README in English](./docs/README_EN.md)**

---

## 📜 Licencia | License

MIT License. Ver / See [`LICENSE`](./LICENSE).

---

## 🛡️ Brand and Logo

"Teluroncio Method", the "TM" logo, and the "renfe-calibracion" project name are distinctive signs created by Francisco Gabriel Murillo Roldán.

The logo and name are protected under a **Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International (CC BY-NC-ND 4.0)** license.

- ✅ You may share and refer to the logo and name in academic, educational, or journalistic contexts, provided you give appropriate credit.
- ❌ You may not use the logo or name for commercial purposes without explicit permission.
- ❌ You may not modify, alter, or create derivative versions of the logo.

Full license: [https://creativecommons.org/licenses/by-nc-nd/4.0/](https://creativecommons.org/licenses/by-nc-nd/4.0/)

---

👤 **Autor | Author:** Francisco Gabriel Murillo Roldán — Ingeniero de Minas | Mining Engineer
🔗 **[LinkedIn](https://linkedin.com/in/tu-perfil)**

---

*Mayo 2026*
